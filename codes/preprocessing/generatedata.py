...
def preprocess_employee_data(df, date_list, outfile_path):
    df = pp.clean_date_format(df, date_list)
    df = pp.derive_datetime(df, date_list)
    df.to_csv(outfile_path, index=False)
    return df

def preprocess_country_data(df, feature_list, nan_replacements, outfile_path):
    df = pp.impute_nan(df, feature_list, nan_replacements=nan_replacements)
    df.to_csv(outfile_path, index=False)
    return df
  
def preprocess_city_data(df, df_us_regions, city_list, col_name, feature_list, nan_replacements, outfile_path):
...
def preprocess_product_data(df, date_list, feature_list, nan_replacements, outfile_path):
    category0='Unknown'
    ...
    items4 = ['Onions', 'Artichoke', 'Grapes', 'Papayas', 'Eggplant', 'Blackberries', 'Kiwi', 'Blueberries', 'Turnip', 'Mangoes',
         'Oranges - Navel', 'Loquat', 'Durian', 'Lettuce', 'Spinach', 'Pomello', 'Fuji Apples', 'Banana', 'Pears',  'Salsify                           Organic', 'Langers - Ruby Red Grapfruit', 'Sprouts - Alfalfa', 'Raspberries - Fresh', 'Beets - Candy Cane Organic',
...
def preprocess_sales_data(df, date_list, outfile_path):
    df = pp.clean_date_format(df, date_list)
